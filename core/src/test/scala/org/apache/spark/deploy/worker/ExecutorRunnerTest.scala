...
class ExecutorRunnerTest extends SparkFunSuite {
  test("command includes appId") {
    ...
    val er = new ExecutorRunner(appId, 1, appDesc, 8, 500, "0,1,2,3,4,5,6,7", null, "blah", "worker321", 123,
      "publicAddr", new File(sparkHome), new File("ooga"), "blah", conf, Seq("localDir"),
      ExecutorState.RUNNING)
    val builder = CommandUtils.buildProcessBuilder(
      appDesc.command, new SecurityManager(conf), 512, "0,1,2,3,4,5,6,7", sparkHome, er.substituteVariables)
